...
import Animated, { add, and, call, clockRunning, cond, eq, greaterThan, neq, not, set, startClock, stopClock, useCode } from 'react-native-reanimated';
...
const BorderTap = ({ children, onPress }: BorderTapProps) => {
    ...
    useCode(
        () => [
            ...
            set(opacity, 
                cond(and(greaterThan(clock, add(start, 100)), clockRunning(clock)), 0.5, 1)
                ),
        ], []);
    return (
